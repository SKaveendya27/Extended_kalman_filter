...
class PoseTimeSeriesPlotter(Node):
    def __init__(self):
        ...
        # Subscribers
        self.create_subscription(Odometry, 'camera/odom/updated', self.gt_callback, 10)
        self.create_subscription(Odometry, '/odometry/filtered', self.ekf_callback, 10)

        # Timer to plot after 10 seconds
        self.create_timer(50.0, self.plot_all)
    ...
    def plot_all(self):
        ...
        labels = ['X Position (m)', 'Y Position (m)', 'Z Position (m)', 'Roll (rad)', 'Pitch (rad)', 'Yaw (rad)']

        for i, field in enumerate(fields):
            ax = axs[i]

            if self.gt_data[field]:
                ax.plot(self.gt_data['t'], self.gt_data[field], label='GT', color='red')
